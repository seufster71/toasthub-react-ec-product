...
export function init({lang}) {
  return function(dispatch) {
    ...
    requestParams.service = "EC_PRODUCT_SVC";
    requestParams.prefTextKeys = new Array("EC_PRODUCT_PAGE");
    requestParams.prefLabelKeys = new Array("EC_PRODUCT_PAGE");
    ...
    return callService(params).then( (responseJson) => {
    	if (responseJson != null && responseJson.protocalError == null){
    		dispatch({ type: "LOAD_INIT_EC_PRODUCT", responseJson });
		} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		dispatch({type:"EC_PRODUCT_PREF_CHANGE", userPrefChange});
		let params = {};
		params.requestParams = requestParams;
		params.URI = '/api/member/callService';
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "LOAD_LIST_EC_PRODUCT", responseJson });
				if (info != null) {
...
export function listLimit({state,listLimit}) {
	return function(dispatch) {
		 dispatch({ type:"EC_PRODUCT_LISTLIMIT",listLimit});
...
export function search({state,searchCriteria}) {
	return function(dispatch) {
		 dispatch({ type:"EC_PRODUCT_SEARCH",searchCriteria});
...
export function modifyItem({id,appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "EC_PRODUCT_SVC";
	    requestParams.prefFormKeys = new Array("EC_PRODUCT_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'EC_PRODUCT_ITEM',responseJson,appPrefs});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function inputChange(field,value) {
	 return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"EC_PRODUCT_INPUT_CHANGE",params});
	 };
}

export function orderBy({state,orderCriteria}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_PRODUCT_ORDERBY",orderCriteria});
...
export function clearItem() {
	return function(dispatch) {
		dispatch({ type:"EC_PRODUCT_CLEAR_ITEM"});
	};
}
...
export function clearField(field) {
	return function(dispatch) {
		let params = {};
		 params.field = field;
		dispatch({ type:"EC_PRODUCT_CLEAR_FIELD",params});
	};
}
export function setErrors({errors}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_PRODUCT_SET_ERRORS",errors});
	 };
}

export function openDeleteModal({item}) {
	 return function(dispatch) {
		 dispatch({type:"EC_PRODUCT_OPEN_DELETE_MODAL",item});
	 };
}

export function closeDeleteModal() {
	 return function(dispatch) {
		 dispatch({type:"EC_PRODUCT_CLOSE_DELETE_MODAL"});
	 };
}